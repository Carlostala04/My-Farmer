...
const MAPTILER_KEY = process.env.EXPO_PUBLIC_MAPTILER_KEY || "vd73viHGHc4ypSX2zv97";
...
export default function MapaParcelaScreen() {
  ...
  const parsePoligono = (pol: any) => {
    ...
    try { return JSON.parse(pol); } catch { return []; }
  };
  ...
  const createParcela = async () => {
    if (!session?.access_token) return Alert.alert("Error", "No has iniciado sesión");
    ...
    if (polygonCoords.length < 3) return Alert.alert("Error", "Debes dibujar un polígono válido con al menos 3 puntos");
        ...
        { text: "Cancelar", style: "cancel" },
        {
          ...
          onPress: async () => {
            ...
            try {
              ...
              Alert.alert("Error", "No se pudo crear la parcela. Verifica la conexión con el servidor.");
            }
          }
        }
      ]
  ...
  const updateParcela = async () => {
    if (!session?.access_token) return Alert.alert("Error", "No has iniciado sesión");
    if (!selectedParcelaId) return Alert.alert("Error", "No hay parcela seleccionada");
    if (!nombre.trim()) return Alert.alert("Error", "El nombre es obligatorio");
    if (!descripcion.trim()) return Alert.alert("Error", "La descripción es obligatoria");
    if (polygonCoords.length < 3) return Alert.alert("Error", "Polígono no válido");

    Alert.alert(
      "Confirmar",
      "¿Deseas guardar los cambios en esta parcela?",
      [
        { text: "Cancelar", style: "cancel" },
        {
          text: "Guardar",
  ...
  const deleteParcela = async () => {
    if (!session?.access_token) return Alert.alert("Error", "No has iniciado sesión");
    if (!selectedParcelaId) return Alert.alert("Error", "Selecciona una parcela para eliminar");
        ...
        <View style={{flex:1, minHeight:600}}>
          ...
          <View style={styles.form}>
            <View style={{flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center', marginBottom: 10}}>
              ...
              <Button title="Salir" color="orange" onPress={handleLogout}/>
            </View>
            ...
            <Text style={{marginBottom:10}}>Área: {area.toFixed(2)} m²</Text>
            
            <View style={styles.buttons}>
              <Button title="Crear" onPress={createParcela}/>
              <Button title="Editar" onPress={updateParcela} disabled={!selectedParcelaId}/>
              <Button title="Eliminar" onPress={deleteParcela} disabled={!selectedParcelaId} color="red"/>
            </View>
          </View>
        </View>
      </ScrollView>
