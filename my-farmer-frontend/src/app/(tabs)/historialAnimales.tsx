/**
 * Pantalla de Historial de Animal (historialAnimales.tsx)
 *
 * Muestra el historial de cambios de un animal específico.
 * Cada registro indica qué campo fue modificado, el valor anterior
 * y el valor nuevo, junto con la fecha del cambio.
 *
 * Recibe `animal_id` y `animal_nombre` como parámetros de navegación.
...
export default function HistorialAnimales() {
  ...
  const { animal_id, animal_nombre } = useLocalSearchParams<{
    animal_id: string;
    animal_nombre: string;
  }>();

  const { historial, loading, error } = useHistorialAnimal(
    animal_id ? Number(animal_id) : null,
  );

  return (
    <>
      <ScreenHeader title={`Historial: ${animal_nombre ?? "Animal"}`} />
      <ScrollView
