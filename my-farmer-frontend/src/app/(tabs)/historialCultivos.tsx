...
export default function HistorialCultivos() {
  ...
  const { historial, loading, error } = useHistorialCultivo(Number(cultivoId))
  ...
  return (
    <View style={styles.container}>
      <Text style={styles.title}>Historial del Cultivo</Text>
      <FlatList
        data={historial}
        keyExtractor={(item) => String(item.Historial_Cultivo_id)}
        renderItem={({ item }) => (
